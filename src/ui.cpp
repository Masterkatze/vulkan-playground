...
namespace vkpg
{
	...
	void UI::prepareResources()
	{
		...
		VK_CHECK_RESULT(vkCreateImage(device->logical_device, &imageInfo, nullptr, &fontImage));
		VkMemoryRequirements memReqs;
		vkGetImageMemoryRequirements(device->logical_device, fontImage, &memReqs);
		VkMemoryAllocateInfo memAllocInfo = vks::initializers::memoryAllocateInfo();
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		VK_CHECK_RESULT(vkAllocateMemory(device->logical_device, &memAllocInfo, nullptr, &fontMemory));
		VK_CHECK_RESULT(vkBindImageMemory(device->logical_device, fontImage, fontMemory, 0));
		...
		VK_CHECK_RESULT(vkCreateImageView(device->logical_device, &viewInfo, nullptr, &fontView));
